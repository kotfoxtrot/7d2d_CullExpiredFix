...
| | |
|---|---|
| game | 7 Days to Die dedicated server **3.x** (measured on 3.1.0 b14) |
...
```bash
git clone -b 3.x https://github.com/kotfoxtrot/7d2d_CullExpiredFix.git
...
difference is that an expired chunk may be removed up to `IntervalSeconds` late — set against a
...
There is no assembly reference between the two — nothing breaks if it is absent, there is just no
point.
...
protection rebuild, group timestamps, scan and removal. Useful for diagnosis, not needed to run this
mod.
