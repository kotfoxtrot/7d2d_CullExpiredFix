        ...
        private static void Run(RegionFileManager rfm)
        {
            ...
            lock (rfm.saveLock)
            {
                lock (rfm.chunksInSaveDir)
                {
                    ...
                    Dictionary<long, HashSetLong> groupsByChunkKey = rfm.groupsByChunkKey;
                    Dictionary<HashSetLong, uint> groupTimestamps = rfm.groupTimestamps;
                    ...
                    if (maxAge >= 0L)
                    {
                        ...
                        bool anyGroups = groupsByChunkKey != null && groupTimestamps != null && groupsByChunkKey.Count > 0;
                        ...
                        foreach (KeyValuePair<long, uint> kv in csd)
                        {
                            ...
                            if (!anyReset || !resetSet.Contains(key))
                            {
                                ...
                                if (anyGroups)
                                {
                                    HashSetLong group;
                                    uint groupTs;
                                    if (groupsByChunkKey.TryGetValue(key, out group) && groupTimestamps.TryGetValue(group, out groupTs))
                                    {
                                        ts = groupTs;
                                    }
                                }
